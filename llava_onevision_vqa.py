...
def run_llava_onevision(
    jsonl_path,
    output_path,
    model_id="./weights/llava-onevision-qwen2-7b-ov-hf",
    limit=None,
    max_new_tokens=4,
    ...
    # 官方建议 batched generation 时使用 left padding
    ...
    for item in tqdm(data, desc="Running LLaVA-OneVision inference"):
        ...
        try:
            ...
            # LLaVA-OneVision 的 chat template 要求 content 中包含 image 和 text
            conversation = [
                {
                    "role": "user",
                    "content": [
            ...
            # 将 tensor 移动到模型所在设备
            # 注意 pixel_values 也需要转为 float16
            inputs = {
                k: (v.to(model.device, dtype=torch.float16) if v.dtype.is_floating_point else v.to(model.device))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="results_llava_onevision_7b.jsonl",
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="./weights/llava-onevision-qwen2-7b-ov-hf",
